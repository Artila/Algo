 ...
 *
 * 第二个参数，func，是一个函数。用来测试数组的第一个元素，如果返回 false，就从数组中抛出该元素(注意：此时数组已被改变)，继续测试数组的第一个元素，如果返回 false，继续抛出，直到返回true。
 *
 * 最后返回数组的剩余部分，如果没有剩余，就返回一个空数组。
 *
 * 题目的意思就是返回数组中第一个满足条件的元素之后的元素。
 */

function drop(arr, func) {
  var filtered = arr.filter(func);
  var temp = [];
  // 数组不为空，则取数组中第一个满足条件的元素之后的元素
  // 数组为空，返回空数组
  if (filtered.length !== 0) {
    var i = arr.indexOf(filtered[0]);
    temp = arr.slice(i);
  }
...
drop([1, 2, 3], function(n) {return n < 3; }); // [ 1, 2, 3 ]
